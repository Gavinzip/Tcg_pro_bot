...
DEFAULT_RENAISS_CARD_API_BASE_URL = "https://api.renaiss.xyz/v0/cards"
...
def fetch_card_api_collectible(
    ...
    *,
    ...
    """Fetch one collectible using the REST endpoint used by renaiss-cli."""
    ...
    # The endpoint rate-limits bursts of unique token requests. Serialize all
    # requests in the process so concurrent profile jobs cannot trigger a WAF block.
    ...
    raise RuntimeError(f"Renaiss card API request failed after {retries} attempts: {last_err}")
